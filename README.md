...
magic-string is a small, fast utility for manipulating strings and generating sourcemaps.

> Note: magic-string is currently being community maintained by [@antfu]().
...
## Why generate sourcemaps?

The [sourcemaps](https://docs.google.com/document/d/1U1RGAehQwRypUTovF1KRlpiOFze0b-_2gc6fAH0KY0k/edit) generated by magic-string avoids a break in the step debugger when doing string manipulations on a generated file.

For example, CLI tools like [Vite](https://vitejs.dev/) and [rollup](https://rollupjs.org/) allow users to write plugins that transform JavaScript files as part of a pipeline... If a generated file throws an error in the broswer context, it helps with debugging if that error can be mapped back to the plugin line of code responsible.

## Why was magic-string created?
